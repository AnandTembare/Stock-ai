...
# Professional Streamlit App | Live NSE/BSE data via yfinance
# ============================================================
...
def fetch_stock_data(ticker: str, period: str = "1y") -> pd.DataFrame:
    try:
        ...
        # Ensure we only keep standard columns
...
def ml_predict(df: pd.DataFrame) -> tuple:
    ...
    # Target: 1 if tomorrow's close is higher than today's close
    ...
    # Predict using the most recent data point (today) to guess tomorrow
    latest_features = df[features].iloc[-1].values.reshape(1, -1)
    prob = model.predict_proba(scaler.transform(latest_features))[0][1]
...
def main():
    ...
    if df.empty:
        st.error(f"Could not fetch data for {stock_name} ({ticker}). Check your internet connection or try another stock.")
        st.stop()
    ...
    try:
        ...
        price, prev, change, change_p = 0.0, 0.0, 0.0, 0.0

    high_52 = float(df["High"].max())
    low_52  = float(df["Low"].min())
    avg_vol = int(df["Volume"].mean())
    rsi_val = float(df_ind["RSI"].iloc[-1]) if "RSI" in df_ind.columns and not pd.isna(df_ind["RSI"].iloc[-1]) else 0.0

    m1, m2, m3, m4, m5, m6 = st.columns(6)
    m1.metric("Live Price", f"₹{price:,.2f}", f"{change:+.2f} ({change_p:+.2f}%)")
    m2.metric("Period High",   f"₹{high_52:,.2f}")
    m3.metric("Period Low",    f"₹{low_52:,.2f}")
    ...
    with tab2:
        ...
        with col_b:
            ...
            hist_colors = ["#22c55e" if v >= 0 else "#ef4444" for v in (df_ind["MACD"] - df_ind["MACD_Signal"])]
        ...
        def macd_signal(m, s):
            if pd.isna(m) or pd.isna(s): return "Unknown"
            return "✅ Bullish (MACD > Signal)" if m > s else "🔴 Bearish (MACD < Signal)"

        def sma_signal(p, sma):
            if pd.isna(sma): return "Unknown"
            return "✅ Bullish (Above SMA)" if p > sma else "🔴 Bearish (Below SMA)"

        summary_data = {
            "Indicator": ["RSI (14)", "MACD", "SMA 20", "SMA 50", "ATR (14)"],
            "Value": [
                f"{latest['RSI']:.2f}" if not pd.isna(latest['RSI']) else "N/A",
                f"{latest['MACD']:.2f}" if not pd.isna(latest['MACD']) else "N/A",
                f"₹{latest['SMA_20']:.2f}" if not pd.isna(latest['SMA_20']) else "N/A",
                f"₹{latest['SMA_50']:.2f}" if not pd.isna(latest['SMA_50']) else "N/A",
                f"₹{latest['ATR']:.2f}" if not pd.isna(latest['ATR']) else "N/A",
            ],
            "Signal": [
                rsi_signal(latest.get("RSI")),
                macd_signal(latest.get("MACD"), latest.get("MACD_Signal")),
                sma_signal(price, latest.get("SMA_20")),
                sma_signal(price, latest.get("SMA_50")),
                "Volatility measure",
            ],
        }
    ...
    # TAB 4: Raw Data
    with tab4:
        st.markdown(f"#### {stock_name} — Raw Trading Data")
        display_df = df.copy()
        # Convert index to a column formatted as string to ensure clean CSV and display
